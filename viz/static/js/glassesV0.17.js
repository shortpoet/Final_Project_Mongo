...
function drawGlass(cocktail_endpoint, chosenRecipe, chosenCateg) {
	d3.json(cocktail_endpoint).then(function(cocktails) {
      ...
      if ( chosenCateg == 'AI Instant Classic') {
...
function drawTable(cocktail_endpoint, chosenParam, paramType) {
    ...
    if (paramType === 'ingredient') {
      ...
      if ( chosenCateg == 'AI Instant Classic') {
        var recipe = cocktails.filter(datum => datum.Category == chosenParam)
        if (recipe.length > 1) {
          recipe = recipe
        }
      }
      else {
        var chosenRecipes = cocktails.filter(datum => datum.Cocktail_Name == chosenRecipe && datum.Category == chosenCateg)
      }
